def findVendaFB_CF_SR(service, options) -> list:
    from selenium import webdriver
    from selenium.webdriver.common.by import By
    try: from Fb_Corretor__Caio_Fernandes__Samuel.main import linksVendaFB_CF_SR
    except: from main import linksVendaFB_CF_SR
    from time import time

    time_start = time()

    links = linksVendaFB_CF_SR(service, options)
    ...
    for link in links:
        ...
        try: 
            ...
            if text_desc != "": infos[8].append(text_desc)
        except: pass
        
        #Valor:
        try: infos[11].append(float(driver.find_element(By. ID, "info__valor").text.replace("R$ ","").replace(".","").replace(",",".")))
        except: pass        
